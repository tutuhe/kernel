...
extern int stmmac_mdio_read_switch(struct mii_bus *bus, int phyaddr,
								   int phyreg);
extern int stmmac_mdio_write_switch(struct mii_bus *bus, int phyaddr,
									int phyreg, u16 phydata);

/* MDC/MDIO, redefine/implement the following Macro */
//#define MDC_MDIO_WRITE(preamableLength, phyID, regID, data)
//rtl8761_mdio_write(phyID,regID, data) #define MDC_MDIO_READ(preamableLength,
//phyID, regID, pData) rtl8761_mdio_read(phyID,regID,pData)

#define MDC_MDIO_WRITE(preamableLength, phyID, regID, data)                    \
	stmmac_mdio_write_switch(rtl8761_bus, phyID, regID, data);

#define MDC_MDIO_READ(preamableLength, phyID, regID, pData)                    \
	pData = stmmac_mdio_read_switch(rtl8761_bus, phyID, regID);
...
rtk_int32 smi_read(rtk_uint32 mAddrs, rtk_uint32 *rData)
{
	...
	MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL0_REG,
				   MDC_MDIO_ADDR_OP);

	/* Write address to register 23 */
	MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_ADDRESS_REG,
				   mAddrs);

	/* Write read control code to register 21 */
	MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL1_REG,
				   MDC_MDIO_READ_OP);

	/* Read data from register 25 */
	MDC_MDIO_READ(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID,
				  MDC_MDIO_DATA_READ_REG, pData);
	*rData = pData;
	// printk("3333333333333333333333-=---------------------%d\n",*rData);
	...
	/*Disable CPU interrupt to ensure that the SMI operation is atomic.
	  The API is based on RTL865X, rewrite the API if porting to other
	  platform.*/
	rtlglue_drvMutexLock();
...
rtk_int32 smi_write(rtk_uint32 mAddrs, rtk_uint32 rData)
{
	...
	MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID,
